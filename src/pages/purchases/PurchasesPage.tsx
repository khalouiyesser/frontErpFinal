...
import {
  ...
  Package, ChevronRight, ChevronLeft, Search, RefreshCw,
  SlidersHorizontal, LayoutGrid, List, Banknote, CheckCircle2,
  Clock, AlertCircle, Building2, Receipt, Calendar,
} from 'lucide-react';
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 border-t border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
          ...
          <select value={pageSize} onChange={e => { onPageSize(+e.target.value); onPage(1); }}
                  className="px-2 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-xs font-medium text-gray-700 dark:text-gray-300 focus:outline-none">
...
const PurchaseCard: React.FC<{
  purchase: any;
  statusConfig: Record<string, { label: string; cls: string; border: string }>;
  onView: () => void;
  onPayment: (e: React.MouseEvent) => void;
  onDownload: (e: React.MouseEvent) => void;
  onDelete: (e: React.MouseEvent) => void;
}> = ({ purchase, statusConfig, onView, onPayment, onDownload, onDelete }) => {
  const { t } = useTranslation();
  const cfg = statusConfig[purchase.status] || { label: purchase.status, cls: 'bg-gray-100 text-gray-500', border: 'border-gray-200' };
  return (
      <div onClick={onView}
        ...
        <div className={`h-1 w-full ${purchase.status === 'paid' ? 'bg-gradient-to-r from-emerald-500 to-emerald-400' : purchase.status === 'partial' ? 'bg-gradient-to-r from-amber-400 to-amber-500' : 'bg-gradient-to-r from-red-400 to-red-500'}`} />
        <div className="p-4">
          <div className="flex items-start justify-between gap-3 mb-3">
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-violet-500 to-violet-600 flex items-center justify-center shrink-0 shadow-sm">
                <span className="text-white text-base font-bold">{(purchase.FournisseurName || '?').charAt(0).toUpperCase()}</span>
              </div>
              <div className="min-w-0">
                <p className="font-bold text-gray-900 dark:text-white text-sm leading-tight truncate">{purchase.FournisseurName}</p>
                <p className="text-xs text-gray-400 mt-0.5">{purchase.createdAt ? format(new Date(purchase.createdAt), 'dd/MM/yyyy') : '—'}</p>
              </div>
            </div>
            <span className={`shrink-0 inline-flex items-center gap-1 text-xs px-2 py-1 rounded-full font-semibold border ${cfg.cls} ${cfg.border}`}>{cfg.label}</span>
          </div>

          <div className="grid grid-cols-3 gap-2 mb-3">
            {[
              { label: t('sales.totalTTC'),       value: formatTND(purchase.totalTTC),        cls: 'text-blue-600 dark:text-blue-400'    },
              { label: t('sales.amountPaid'),      value: formatTND(purchase.amountPaid),      cls: 'text-emerald-600 dark:text-emerald-400' },
              { label: t('sales.amountRemaining'), value: formatTND(purchase.amountRemaining), cls: purchase.amountRemaining > 0 ? 'text-red-500' : 'text-gray-400' },
            ].map(({ label, value, cls }) => (
                <div key={label} className="bg-gray-50 dark:bg-gray-800/50 rounded-lg px-2.5 py-1.5 text-center">
          ...
          <div className="flex items-center gap-1.5 pt-3 border-t border-gray-100 dark:border-gray-800">
            <button onClick={e => { e.stopPropagation(); onView(); }}
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold bg-gray-50 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-700 transition-colors">
              <Eye size={12} /> {t('common.view')}
            </button>
            {purchase.status !== 'paid' && (
                <button onClick={e => { e.stopPropagation(); onPayment(e); }}
                        className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800 transition-colors">
                  <Banknote size={12} /> {t('sales.addPayment')}
                </button>
            )}
            <button onClick={e => { e.stopPropagation(); onDownload(e); }}
                    className="p-2 rounded-xl bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/30 border border-indigo-200 dark:border-indigo-800 transition-colors">
              <FileText size={13} />
            </button>
            <button onClick={e => { e.stopPropagation(); onDelete(e); }}
                    className="p-2 rounded-xl bg-red-50 dark:bg-red-900/20 text-red-500 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/30 border border-red-200 dark:border-red-800 transition-colors">
              <Trash2 size={13} />
            </button>
          </div>
        </div>
  ...
  return (
      <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
        ...
        <h3 className="text-base font-bold text-gray-900 dark:text-white mb-1">
          {hasFilters ? t('common.noData') : t('nav.purchases')}
        </h3>
        <p className="text-sm text-gray-500 dark:text-gray-400 max-w-xs mb-5">
          {hasFilters ? t('common.reset') : t('common.noData')}
        </p>
        {!hasFilters && (
...
const SkeletonCard = () => (
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-4 animate-pulse">
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">{[1,2,3].map(i => <div key={i} className="h-10 bg-gray-100 dark:bg-gray-800 rounded-lg" />)}</div>
...
const PurchasesPage: React.FC = () => {
  ...
  const [form, setForm] = useState({
    FournisseurId: '', items: [emptyItem()], initialPayment: 0, paymentMethod: 'cash', notes: '',
  });
  ...
  const supplierProductIds = (selectedSupplier?.products || []).map((p: any) => typeof p === 'string' ? p : p._id?.toString());
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(t('purchases.new') || t('common.create'));
      setShowForm(false);
  ...
  const paymentMut = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowPayment(null); setPayAmount(''); setPayNote('');
    },
  });
  ...
  const handleDelete = useCallback((p: any) => {
    confirm(
        { title: `${t('common.delete')} "${p.FournisseurName}"`, message: `${t('common.delete')} ${formatTND(p.totalTTC)} ?`, dangerMessage: t('common.action_irreversible'), confirmLabel: t('common.delete') },
        () => deleteMut.mutate(p._id),
    );
  }, [confirm, deleteMut, t]);

  const handleDeletePayment = useCallback((purchaseId: string, paymentId: string, amount: number) => {
    confirm(
        { title: t('common.delete'), message: `${t('common.delete')} ${formatTND(amount)} ?`, confirmLabel: t('common.delete') },
  ...
  const handleAddPayment = () => {
    ...
    paymentMut.mutate({ id: showPayment._id, data: { amount: +payAmount, method: payMethod, note: payNote } });
  };
  ...
  const selectProduct = (i: number, productId: string) => {
    ...
    setForm(f => ({
      ...
      items: f.items.map((item, idx) =>
          idx === i ? { ...item, productId: p?._id || '', productName: p?.name || '', unitPrice: p?.purchasePrice || 0, tva: p?.tva ?? 19 } : item,
      ),
    }));
  };
  ...
  const paidCount     = allPurchases.filter(p => p.status === 'paid').length;
  ...
  const filtered = useMemo(() => {
    ...
    return allPurchases.filter(p => {
      if (q && !((p.FournisseurName || '').toLowerCase().includes(q))) return false;
      if (statusFilter !== 'all' && p.status !== statusFilter) return false;
      return true;
    });
  }, [allPurchases, search, statusFilter]);
  ...
  const columns = [
    ...
    { key: 'totalTTC',        header: t('sales.totalTTC'),       sortable: true, render: (v: number) => <span className="text-sm font-semibold text-blue-600 dark:text-blue-400">{formatTND(v)}</span> },
    { key: 'amountPaid',      header: t('sales.amountPaid'),     sortable: true, className: 'hidden sm:table-cell', render: (v: number) => <span className="text-sm text-emerald-600 dark:text-emerald-400 font-medium">{formatTND(v)}</span> },
    { key: 'amountRemaining', header: t('sales.amountRemaining'), sortable: true, className: 'hidden md:table-cell', render: (v: number) => <span className={`text-sm font-semibold ${v > 0 ? 'text-red-500' : 'text-gray-400'}`}>{formatTND(v)}</span> },
    {
    ...
    { key: 'createdAt', header: t('common.date'), sortable: true, className: 'hidden lg:table-cell', render: (v: string) => v ? <span className="text-xs text-gray-400">{format(new Date(v), 'dd/MM/yyyy')}</span> : <span className="text-gray-300">—</span> },
  ];
  ...
  return (
      <div className="space-y-4 sm:space-y-5" dir={dir}>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white tracking-tight">{t('nav.purchases')}</h1>
            <p className="text-gray-400 dark:text-gray-500 text-xs sm:text-sm mt-0.5">
              {filtered.length !== allPurchases.length
                  ? <><span className="text-blue-500 font-semibold">{filtered.length}</span> / {allPurchases.length} {t('nav.purchases').toLowerCase()}</>
                  : <>{allPurchases.length} {t('nav.purchases').toLowerCase()}</>}
              {pendingCount > 0 && !hasFilters && <span className="text-red-500 ms-1.5">· {pendingCount} {t('sales.status.pending').toLowerCase()}</span>}
            </p>
          </div>
          <div className="flex items-center gap-2 self-start sm:self-auto">
            ...
            <button onClick={() => setShowForm(true)}
              ...
              <Plus size={16} /> {t('purchases.new') || t('nav.purchases')}
            </button>
          </div>
        </div>
        ...
        {allPurchases.length > 0 && (
            <div className="flex gap-2 flex-wrap">
              <button onClick={() => handleStatusFilter('all')}
                ...
                {t('common.all')} ({allPurchases.length})
              </button>
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
          <SearchBar value={search} onChange={handleSearch} placeholder={`${t('common.search')} ${t('nav.purchases').toLowerCase()}…`} />
          <div className="flex items-center gap-2 shrink-0">
            <div className="hidden sm:flex items-center gap-0.5 p-1 rounded-xl bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
              <button onClick={() => setViewMode('table')} className={`p-1.5 rounded-lg transition-all ${viewMode === 'table' ? 'bg-white dark:bg-gray-700 shadow-sm text-blue-600' : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'}`}><List size={15} /></button>
              <button onClick={() => setViewMode('cards')} className={`p-1.5 rounded-lg transition-all ${viewMode === 'cards' ? 'bg-white dark:bg-gray-700 shadow-sm text-blue-600' : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'}`}><LayoutGrid size={15} /></button>
            </div>
          </div>
        </div>
        ...
        {hasFilters && (
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1"><SlidersHorizontal size={12} /> {t('common.filter')} :</span>
              {search && (
              ...
              <button onClick={() => { handleSearch(''); handleStatusFilter('all'); }} className="text-xs text-red-500 hover:text-red-600 font-medium underline">{t('common.reset')}</button>
            </div>
        )}
        ...
        {isLoading ? (
            ...
            <>
              {/* Cards — mobile toujours, desktop si viewMode=cards */}
              <div className={`${viewMode === 'cards' ? 'block' : 'block sm:hidden'}`}>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {paginated.map(p => (
                      <PurchaseCard key={p._id} purchase={p} statusConfig={statusConfig}
                                    onView={() => setDetailPurchase(p)}
                                    onPayment={e => { e.stopPropagation(); setShowPayment(p); }}
                                    onDownload={e => { e.stopPropagation(); handleDownloadInvoice(p._id); }}
              ...
              {/* Table — desktop si viewMode=table */}
              {viewMode === 'table' && (
                  <div className="hidden sm:block">
                    <DataTable data={paginated} columns={columns} searchable={false} searchKeys={[]}
                               isLoading={false} emptyMessage={t('common.noData')} onRowClick={setDetailPurchase}
                               actions={row => (
                                   <div className="flex items-center justify-end gap-0.5">
                                     <button onClick={e => { e.stopPropagation(); setDetailPurchase(row); }} className="p-1.5 text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors"><Eye size={14} /></button>
                                     <button onClick={e => { e.stopPropagation(); handleDownloadInvoice(row._id); }} className="p-1.5 text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-lg transition-colors"><FileText size={14} /></button>
                                     {row.status !== 'paid' && (
                                         <button onClick={e => { e.stopPropagation(); setShowPayment(row); }} className="p-1.5 text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded-lg transition-colors"><CreditCard size={14} /></button>
                                     )}
                                     <button onClick={e => { e.stopPropagation(); handleDelete(row); }} className="p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"><Trash2 size={14} /></button>
              ...
              <div className="relative bg-white dark:bg-gray-900 rounded-t-3xl sm:rounded-2xl shadow-2xl w-full sm:max-w-2xl mx-0 sm:mx-4 max-h-[95vh] sm:max-h-[90vh] flex flex-col overflow-hidden">
                ...
                <div className="px-5 sm:px-6 pt-3 sm:pt-5 pb-4 border-b border-gray-100 dark:border-gray-800 shrink-0 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-base font-bold text-gray-900 dark:text-white">{t('purchases.new') || t('nav.purchases')}</h2>
                      <p className="text-xs text-gray-400 mt-0.5">{t('nav.suppliers')}</p>
                    </div>
                  </div>
                ...
                <form onSubmit={e => { e.preventDefault(); createMut.mutate(form as any); }} className="flex flex-col flex-1 overflow-hidden">
                  <div className="flex-1 overflow-y-auto px-5 sm:px-6 py-5 space-y-5">
                    ...
                    {/* Fournisseur */}
                    <div>
                      <label className={labelCls}>{t('nav.suppliers')} <span className="text-red-400 normal-case font-normal">*</span></label>
                      <div className="flex gap-2">
                        <select required value={form.FournisseurId} onChange={e => setForm(f => ({ ...f, FournisseurId: e.target.value, items: [emptyItem()] }))} className={inp + ' flex-1'}>
                          <option value="">{t('common.search')}</option>
                          {(suppliers as any[]).map((s: any) => <option key={s._id} value={s._id}>{s.name} — {s.phone}</option>)}
                        </select>
                        <button type="button" onClick={() => setShowAddSupplierForm(true)}
                          ...
                          <Plus size={12} /> {t('suppliers.new')}
                        </button>
                      </div>
                      {form.FournisseurId && supplierProducts.length === 0 && (
                          <p className="text-xs text-amber-600 dark:text-amber-400 mt-1.5">⚠️ {t('common.noData')}</p>
                      )}
                    </div>
                        ...
                        <table className="w-full text-xs">
                          ...
                          {form.items.map((item, i) => (
                              <tr key={i}>
                                ...
                                <td className="px-2 py-2"><input type="number" min={1} value={item.quantity} onChange={e => setForm(f => ({ ...f, items: f.items.map((x, j) => j === i ? { ...x, quantity: +e.target.value } : x) }))} className="w-full px-2 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-xs text-end text-gray-900 dark:text-white focus:outline-none" /></td>
                                <td className="px-2 py-2"><input type="number" min={0} step={0.001} value={item.unitPrice} onChange={e => setForm(f => ({ ...f, items: f.items.map((x, j) => j === i ? { ...x, unitPrice: +e.target.value } : x) }))} className="w-full px-2 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-xs text-end text-gray-900 dark:text-white focus:outline-none" /></td>
                                <td className="px-2 py-2"><input type="number" min={0} value={item.tva} onChange={e => setForm(f => ({ ...f, items: f.items.map((x, j) => j === i ? { ...x, tva: +e.target.value } : x) }))} className="w-full px-2 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-xs text-end text-gray-900 dark:text-white focus:outline-none" /></td>
                                <td className="px-3 py-2 text-end font-medium text-gray-700 dark:text-gray-300">{(item.quantity * item.unitPrice * (1 + item.tva / 100)).toFixed(3)}</td>
                                <td className="pe-2 py-2">{form.items.length > 1 && (<button type="button" onClick={() => setForm(f => ({ ...f, items: f.items.filter((_, j) => j !== i) }))} className="text-red-400 hover:text-red-600 p-1 rounded"><X size={12} /></button>)}</td>
                              </tr>
                          ))}
                          </tbody>
                        </table>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <div>
                        ...
                        <input type="number" min={0} step={0.001} max={totalTTC} value={form.initialPayment} onChange={e => setForm(f => ({ ...f, initialPayment: +e.target.value }))} className={inpSm} />
                        {form.initialPayment > 0 && (
                            <p className="text-xs text-gray-400 mt-1">
                              {t('sales.amountRemaining')} : <span className={totalTTC - form.initialPayment > 0 ? 'text-red-500 font-medium' : 'text-emerald-500 font-medium'}>{formatTND(Math.max(0, totalTTC - form.initialPayment))}</span>
                            </p>
                        )}
                      </div>
                      <div>
              ...
              <div className="relative bg-white dark:bg-gray-900 rounded-t-3xl sm:rounded-2xl shadow-2xl w-full sm:max-w-sm mx-0 sm:mx-4 overflow-hidden">
                ...
                <div className="px-5 pt-4 pb-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-sm font-bold text-gray-900 dark:text-white">{t('suppliers.new')}</h3>
                  </div>
                  <button onClick={() => setShowAddSupplierForm(false)} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl transition-colors text-gray-400"><X size={16} /></button>
                </div>
                <div className="p-5 space-y-4">
                  <div>
                    <label className={labelCls}>{t('common.name')} <span className="text-red-400 normal-case font-normal">*</span></label>
                    <input value={newSupplierForm.name} onChange={e => setNewSupplierForm(f => ({ ...f, name: e.target.value }))} placeholder={t('common.optional')} className={inp} autoFocus />
                  </div>
                  <div>
                    <label className={labelCls}>{t('common.phone')} <span className="text-red-400 normal-case font-normal">*</span></label>
                    <input value={newSupplierForm.phone} onChange={e => setNewSupplierForm(f => ({ ...f, phone: e.target.value }))} pattern="^\+216[0-9]{8}$" className={inp} />
                    <p className="text-xs text-gray-400 mt-1">Format : +216 suivi de 8 chiffres</p>
                  </div>
                  <div className="flex gap-2 pt-1">
                    <button type="button" onClick={() => setShowAddSupplierForm(false)}
                    ...
                    <button type="button" onClick={() => { if (newSupplierForm.name && newSupplierForm.phone) createSupplierMut.mutate(newSupplierForm as any); }}
                            disabled={!newSupplierForm.name || createSupplierMut.isPending}
                            className="flex-1 px-4 py-2.5 bg-violet-600 hover:bg-violet-700 disabled:bg-violet-400 disabled:cursor-not-allowed text-white rounded-xl text-sm font-semibold transition-colors shadow-sm">
                      {createSupplierMut.isPending ? t('common.saving') : t('common.create')}
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="relative bg-white dark:bg-gray-900 rounded-t-3xl sm:rounded-2xl shadow-2xl w-full sm:max-w-md mx-0 sm:mx-4 overflow-hidden">
                ...
                <div className="px-5 pt-4 pb-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-sm font-bold text-gray-900 dark:text-white">{t('sales.addPayment')}</h2>
                      <p className="text-xs text-gray-400 mt-0.5">{showPayment.FournisseurName}</p>
                    </div>
                  </div>
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="number" min={0.001} step={0.001} max={showPayment.amountRemaining} value={payAmount} onChange={e => setPayAmount(e.target.value)}
                    ...
                    {payAmount && parseFloat(payAmount) > 0 && <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-1 ms-1 font-medium">✓ {formatTND(parseFloat(payAmount))}</p>}
                  </div>
                  <div>
                    <label className={labelCls}>{t('sales.paymentMethod')}</label>
                  ...
                  <div className="flex gap-2 pt-1">
                    ...
                    <button onClick={handleAddPayment} disabled={!payAmount || paymentMut.isPending}
                      ...
                      {paymentMut.isPending ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />{t('common.saving')}</> : <><Banknote size={15} />{t('common.save')}</>}
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="relative w-full sm:w-auto sm:max-w-xl sm:h-full bg-white dark:bg-gray-900 shadow-2xl overflow-y-auto rounded-t-2xl sm:rounded-none max-h-[92vh] sm:max-h-full">
                ...
                <div className="sticky top-0 z-10 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between mt-1 sm:mt-0">
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      <h2 className="text-base sm:text-lg font-bold text-gray-900 dark:text-white">
                        {purchaseDetail?.FournisseurName || detailPurchase.FournisseurName}
                      </h2>
                      {(() => { const cfg = statusConfig[purchaseDetail?.status || detailPurchase.status]; return cfg ? <span className={`text-xs px-2 py-0.5 rounded-full font-semibold border ${cfg.cls} ${cfg.border}`}>{cfg.label}</span> : null; })()}
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => { setDetailPurchase(null); setSelectedItem(null); }} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors text-gray-400"><X size={18} /></button>
                  </div>
                </div>
                ...
                <div className="p-4 sm:p-6 space-y-4 sm:space-y-5">
                  ...
                  <div className="grid grid-cols-3 gap-2 sm:gap-3">
                    {[
                      ...
                      { label: t('sales.amountPaid'),     value: formatTND(purchaseDetail?.amountPaid || detailPurchase.amountPaid), cls: 'text-emerald-600 dark:text-emerald-400' },
                      { label: t('sales.amountRemaining'),value: formatTND(purchaseDetail?.amountRemaining ?? detailPurchase.amountRemaining), cls: (purchaseDetail?.amountRemaining ?? detailPurchase.amountRemaining) > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-400' },
                    ].map(({ label, value, cls }) => (
                  ...
                  <div>
                    <h3 className="text-xs sm:text-sm font-semibold text-gray-900 dark:text-white mb-2 sm:mb-3 flex items-center gap-2">
                      <Package size={14} className="text-gray-400" /> {t('products.title')} ({(purchaseDetail?.items || []).length})
                    </h3>
